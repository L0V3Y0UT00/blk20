...
# Bulk Downloader created with (love) by Ans Raza (0xansr)
# Date: July 20, 2025
# Version: v0.21
# Contact: +92 308 6671782
...
if [[ -n "$channel_url" ]]; then
    ...
    # Get channel JSON with debug output
    echo -e "${YELLOW}Running yt-dlp command...${NC}"
    channel_json=$(yt-dlp --flat-playlist --dump-single-json "$channel_url" 2> yt-dlp-error.log)
    yt_dlp_exit_code=$?

    # Check if yt-dlp command failed
    if [[ $yt_dlp_exit_code -ne 0 ]]; then
        echo -e "${RED}yt-dlp failed with exit code $yt_dlp_exit_code. Error log:${NC}"
    ...
    echo -e "${YELLOW}Raw JSON output ( first 500 characters):${NC}"
    echo "${channel_json:0:500}"
    echo -e "${YELLOW}Full JSON saved to channel_json.txt for inspection${NC}"
    ...
    # Extract URLs with debug output
    echo -e "${YELLOW}Extracting URLs with jq...${NC}"
    urls=$(echo "$channel_json" | jq -r '.entries[]?.url // empty')
    if [[ -z "$urls" ]]; then
        echo -e "${RED}No URLs found in JSON. Checking if entries exist...${NC}"
        has_entries=$(echo "$channel_json" | jq '.entries')
        echo -e "${YELLOW}Entries field:${NC} $has_entries"
        exit 1
    fi

    # Process URLs
    echo -e "${YELLOW}Processing URLs...${NC}"
    echo "$urls" | sed 's|^/shorts/|https://www.youtube.com/shorts/|' > "$output_file"
    echo -e "${YELLOW}URLs saved to $output_file:${NC}"
    cat "$output_file"
    ...
    if [[ -s "$output_file" ]]; then
        ...
        echo -e "${RED}No URLs found. Please check the channel URL or JSON structure.${NC}"
        exit 1
    fi
fi
