...
if [[ "$ENVIRONMENT" == "termux" && ! -d "$STORAGE_DIR" ]]; then
...
install_if_missing() {
    if ! command -v "$1" &>/dev/null; then
        ...
        if [[ "$ENVIRONMENT" == "termux" ]]; then
            pkg update -y && $INSTALL_CMD "$1"
        else
            sudo apt update -y && $INSTALL_CMD "$1"
        fi
    fi
}
...
# ========== Prompt for YouTube Channel ==========
echo
read -p "Enter the full YouTube channel URL (e.g., https://www.youtube.com/@TechnoMindHindi-r9u): " channel_url

if [[ -n "$channel_url" ]]; then
    echo -e "${GREEN}Extracting video URLs from channel...${NC}"

    # Remove /shorts if accidentally included
    channel_url_clean=$(echo "$channel_url" | sed 's|/shorts||')

    # Get channel JSON
    channel_json=$(yt-dlp --flat-playlist --dump-single-json "$channel_url_clean")
    ...
    # Extract ONLY Shorts URLs
    echo "$channel_json" | jq -r '.entries[].url' | grep '^/shorts/' | sed 's|^/shorts/|https://www.youtube.com/shorts/|' > "$output_file"

    if [[ -s "$output_file" ]]; then
        count=$(wc -l < "$output_file")
        full_path=$(realpath "$output_file")
        echo -e "${GREEN}Saved $count Shorts URLs to ${output_file}${NC}"
        echo -e "${YELLOW}Path:${NC} $full_path"
    else
        echo -e "${RED}No Shorts URLs found. Please check the channel URL.${NC}"
        exit 1
    fi
fi
...
echo -e "${YELLOW}[$(date +'%Y-%m-%d %H:%M:%S')] Starting download in $ENVIRONMENT environment...${NC}"
yt-dlp -o "${download_dir}/%(title).80s.%(ext)s" -a "$tmp_file"
rm -f "$tmp_file"
echo -e "\n${GREEN}[$(date +'%Y-%m-%d %H:%M:%S')] Download completed. Saved to '${download_dir}'${NC}"
