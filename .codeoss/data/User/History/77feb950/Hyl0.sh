...
if [[ -n "$channel_url" ]]; then
    ...
    if ! channel_json=$(yt-dlp --flat-playlist --dump-single-json "$channel_url" 2>/dev/null); then
        echo -e "${RED}Failed to fetch channel. Please check the URL or try again.${NC}"
        exit 1
    fi

    channel_title=$(echo "$channel_json" | jq -r '.title' | sed 's/[^a-zA-Z0-9_-]/_/g')
    channel_title=${channel_title:-"YouTube_Channel"}
    ...
    if [[ -s "$output_file" ]]; then
        ...
        echo -e "${GREEN}Saved $count video URLs to $output_file${NC}"
...
echo -e "\n${YELLOW}Please preview and choose a valid range from above list.${NC}"
read -p "Enter ${range_label} video range (e.g., 5-15): " range_input
